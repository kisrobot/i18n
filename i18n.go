...
import (
	...
	"github.com/kisrobot/cache/memory"
...
func RenderInlineEditAssets(isIncludeJQuery bool, isIncludeExtendAssetLib bool) (template.HTML, error) {
	for _, gopath := range utils.GOPATH() {
		...
		if isIncludeExtendAssetLib {
			if extendLib, err := ioutil.ReadFile(filepath.Join(gopath, "src/github.com/kisrobot/i18n/views/themes/i18n/inline-edit-libs.tmpl")); err == nil {
				content += string(extendLib)
			} else {
				hasError = true
			}

			if css, err := ioutil.ReadFile(filepath.Join(gopath, "src/github.com/kisrobot/i18n/views/themes/i18n/assets/stylesheets/i18n-inline.css")); err == nil {
		...
		if js, err := ioutil.ReadFile(filepath.Join(gopath, "src/github.com/kisrobot/i18n/views/themes/i18n/assets/javascripts/i18n-inline.js")); err == nil {
...
func (i18n *I18n) ConfigureQorResource(res resource.Resourcer) {
	if res, ok := res.(*admin.Resource); ok {
		...
		res.GetAdmin().RegisterViewPath("github.com/kisrobot/i18n/views")
	}
}
